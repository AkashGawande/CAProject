...
function GetGSTTAXPayer(FID, SDate, EDate, SearchText) {
            ...
            if (r.d != "") {
                $.each(r.d, function (key, value) {
                    ...
                    var rows = "<tr class='odd gradeX'>"
                        ...
                        + "<button id='btnGSTPReply' title='Reply to Franchisee' class='btn btn-success btn-xs'  data-toggle='modal' data-target='#myModalReply'><i class='ace-icon fa fa-reply  bigger-110 icon-only'></i></button>&nbsp;"
...
function GetGSTTAXDeductor(FID, SDate, EDate, SearchText) {
    $("#loaderHdClientReg").show();
            ...
            if (r.d != "") {
                $.each(r.d, function (key, value) {
                    ...
                    var rows = "<tr class='odd gradeX'>"
                        ...
                        + "<button id='btnGSTDReply' title='Reply to Franchisee' class='btn btn-success btn-xs'  data-toggle='modal' data-target='#myModalReply'><i class='ace-icon fa fa-reply  bigger-110 icon-only'></i></button>&nbsp;"
...
function ViewGSTP(ClientID) {
    $("#loaderHdClientReg").show();
    $("#tblGSTPClientListfooter tr").remove();
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            var footer_rows = "<tr><td colspan='" + count + "' style='text-align:left;'><a class='btn btn-sm btn-info' id='btnGSTPAllDownLoad' href='#' ><i class='ace-icon fa fa-download fa-2x bigger-110 icon-only'></i> Download All</a></td></tr>";
            $('#tblGSTPClientListfooter').append(footer_rows);
            $("#loaderHdClientReg").hide();
        },
        failure: function () {
            $("#loaderHdClientReg").hide();
...
function ViewGSTDDoc(ClientID) {
    ...
    $("#tblGSTDClientListfooter tr").remove();
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            var footer_rows = "<tr><td colspan='" + count + "' style='text-align:left;'><a class='btn btn-sm btn-info' id='btnGSTDAllDownLoad' href='#' ><i class='ace-icon fa fa-download fa-2x bigger-110 icon-only'></i> Download All</a></td></tr>";
            $('#tblGSTDClientListfooter').append(footer_rows);
            $("#loaderHdClientReg").hide();
        },
        failure: function () {
            $("#loaderHdClientReg").hide();
...
$(document).on('click', '#btnGSTPAllDownLoad', function () {
    var $row = $(this).closest("tr");
    $($row.find("#btnDownload").remove());
    var CID = $('#txtGSTPClientID').val();
    var ClientName = $('#txtGSTPApplicanteName').val();

    var data = '{ClientID:"' + CID +
        '",ClientName:"' + ClientName + '",TableName:"clientgstpayerdocument"}';
...
//=======================Start Download All Documents=============================
$(document).on('click', '#btnGSTDAllDownLoad', function () {
    var $row = $(this).closest("tr");
    $($row.find("#btnDownload").remove());
    var CID = $('#txtGSTDClientID').val();
    var ClientName = $('#txtGSTDOfficeName').val();

    var data = '{ClientID:"' + CID +
        '",ClientName:"' + ClientName + '",TableName:"clientgstdeductordocument"}';
