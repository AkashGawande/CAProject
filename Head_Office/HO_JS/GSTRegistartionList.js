...
$(document).ready(function () {
    ...
    BindFranchiseeID($('#ddlFranchiseeIDTP'), "GSTRegistartionList.aspx/BindFranchiseeIDP");
    BindFirmName($('#ddlFirmNameTP'), "GSTRegistartionList.aspx/BindFirmNameP");

    BindFranchiseeID($('#ddlFranchiseeIDTD'), "GSTRegistartionList.aspx/BindFranchiseeIDD");
    BindFirmName($('#ddlFirmNameTD'), "GSTRegistartionList.aspx/BindFirmNameD");
      
    //===================================Start Bind FranchiseeID Drop Down=============================
    function BindFranchiseeID(dept,customurlID) {       
        $.ajax({
            type: "POST",
            url: customurlID,
            data: '{}',
    ...
    $('#ddlFranchiseeIDTP').change(function () {
        $('#ddlFirmNameTP').val($('#ddlFranchiseeIDTP').val());
        GetGSTRegTP($('#ddlFranchiseeIDTP').val());
        $('#tblGSTRegTP').show();
    });
    //===================================End Get Get GST Tax Payer on FranchiseeID DDL Change=============================

    //===================================Start Get GST Tax Payer on Firm name DDL Change=============================
    $('#ddlFirmNameTP').change(function () {
        $('#ddlFranchiseeIDTP').val($('#ddlFirmNameTP').val());
        GetGSTRegTP($('#ddlFirmNameTP').val());
        $('#tblGSTRegTP').show();
    });
    //===================================End Get GST Tax Payer on Firm name DDL Change=============================
     
    //===================================Start Get GST Tax Deductor on FranchiseeID DDL Change=============================
    $('#ddlFranchiseeIDTD').change(function () {
        $('#ddlFirmNameTD').val($('#ddlFranchiseeIDTD').val());
        GetGSTRegTD($('#ddlFranchiseeIDTD').val());
        $('#tblGSTRegTD').show();
    });
    //===================================End Get Get GST Tax Deductor on FranchiseeID DDL Change=============================

    //===================================Start Get GST Tax Deductor on Firm name DDL Change=============================
    $('#ddlFirmNameTD').change(function () {
        $('#ddlFranchiseeIDTD').val($('#ddlFirmNameTD').val());
        GetGSTRegTD($('#ddlFirmNameTD').val());
        $('#tblGSTRegTD').show();
    });
...
function ViewTP(RefID) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            var footer_rows = "<tr><td colspan='10' style='text-align:left;'><a class='btn btn-sm btn-info' id='btnGSTPRegAllDownLoad' href='#' ><i class='ace-icon fa fa-download fa-2x bigger-110 icon-only'></i> Download All</a></td></tr>";
            $('#tblGSTPRegfooter').append(footer_rows);
            $("#loaderHdGST").hide();
        },
        failure: function () {
            $("#loaderHdGST").hide();
...
function ViewTD(RefID) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            var footer_rows = "<tr><td colspan='10' style='text-align:left;'><a class='btn btn-sm btn-info' id='btnGSTDRegAllDownLoad' href='#' ><i class='ace-icon fa fa-download fa-2x bigger-110 icon-only'></i> Download All</a></td></tr>";
            $('#tblGSTDRegfooter').append(footer_rows);
        },
        failure: function () {
        }
    });
...
$(document).on('click', '#btnGSTPRegAllDownLoad', function () {
    var $row = $(this).closest("tr");
    $($row.find("#btnDownload").remove());
    var RefID = $('#txtTPReferenceID').val();
    var ClientName = $('#txtTPApplicanteName').val();

    var data = '{RefID:"' + RefID +
        '",ClientName:"' + ClientName + '",TableName:"gstregistrationdocuments"}';

    $.ajax({
        type: "post",
        url: "GSTRegistartionList.aspx/DownloadAllGSTDocument",
        data: data,
        contentType: "application/json",
        dataType: "json",
...
//=======================Start Download All Documents=============================
$(document).on('click', '#btnGSTDRegAllDownLoad', function () {
    var $row = $(this).closest("tr");
    $($row.find("#btnDownload").remove());
    var RefID = $('#txtTDReferenceID').val();
    var ClientName = $('#txtTDOfficeName').val();
